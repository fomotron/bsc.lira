...
function InitMyWallet() {
    ...
    waitForGlobal(initWallet);

}
var networks_index = 56; //正式是56
...
function initWallet() {
    ...
    contract_address = artifact.networks[networks_index].address
    if (state.networkType != "local") {
        web3_obj.eth.getAccounts().then(address => {
            if (address.length <= 0) {
                ...
                web3_obj.eth.getBalance(address[0]).then((Balance) => {
                    ...
                    $(".header_right strong").html(math.round(Balance_format, 6) + " BNB ");
        ...
        //当前用户地址
        $(".header_right p").html(show_msg);
        $(".header_right strong").html("Loading BNB ");
...
function ask_bulk_price(count) {
    contract.methods.ask_bulk_price(count).call().then(output => {
        WriteLog("当前游戏信息:" + JSON.stringify(output));

        var bulk_price = web3_obj.utils.fromWei(output.total_price, 'ether');
        $("span.info_price_bulk").html(math.round(bulk_price, 6));
    });
}
...
function BulkBuyToken() {
    ...
    var buy_count = parseInt($("#txt_buy_count").val());
    if (isNaN(buy_count)) {
        alert("请输入正确的购买次数");
        return;
    }
    ...
    if (buy_count == 1) {
        contract.methods.askprice().call().then(output => {
            ...
            $("span.info_price").html(math.round(state.price, 6));
            $("input.btn_price").val(math.round(state.price, 6));
            $(".dangqianjiagge span").html(math.round(state.price, 6) + "BNB");
            WriteLog(output, "当前价格:" + math.round(state.price, 6) + "BNB");
        ...
        contract.methods.ask_bulk_price(buy_count).call().then(output => {
            var bulk_price = web3_obj.utils.fromWei(output.total_price, 'ether');
            contract.methods.bulk_buy_token(recommend_user, buy_count).send({ from: state.address, value: web3_obj.utils.toWei(bulk_price, 'ether') }, function(error, transactionHash) {
                InitPage();
            });
        });
    }
...
function BindPage(output) {
    ...
    //最后购买用户+最后购买时间
    var show_msg = "还没有人购买";
    if (web3_obj.utils.isAddress(output.last_buy_user_address)) {
        var last_buy_user_address = output.last_buy_user_address;
    ...
    var prize_value = web3_obj.utils.fromWei(output.prize_value, 'ether');


    $("#prize_value").html(math.round(prize_value, 6));
    $(".jiangchi span").html(math.round(prize_value, 6) + "&nbsp;BNB");




    var buy_count = output.buy_count;

    $("#join_member").html(buy_count);
    var bonus_value = web3_obj.utils.fromWei(output.bonus_value, 'ether');

    $("#total_bonus").html(math.round(bonus_value, 6));
    var last_info = show_msg;
    $("#last_user").html(last_info);
    $("#last_time").html(new Date(parseInt(output.last_buy_time) * 1000));

    var price = web3_obj.utils.fromWei(output.price, 'ether');
    state.price = price;
    $("span.info_price").html(math.round(price, 6));
    $("span.info_price_bulk").html(math.round(price, 6));

    $("input.btn_price").val(math.round(price, 6));
    $(".dangqianjiagge span").html(math.round(price, 6) + "&nbsp;BNB");

}

function InitPage() {

    $(".buy_now").unbind('click').bind("click", BuyToken);
    $(".buy_now_bulk").unbind('click').bind("click", BulkBuyToken);
    ...
    if (web3_obj.utils.isAddress(state.address)) {
        contract.methods.getBuerIncomeInvestment(state.address).call().then(output => {
            ...
            $("#user_income_value").html(math.round(income, 6));
            $("#user_investment_value").html(math.round(investment, 6));
            $("#user_recommend_value").html(math.round(recommend_value, 6));
            if (user_have_count > 0) {
                $(".buy_now span.btn__text").html("立即复投");
                $(".buy_now_bulk span.btn__text").html("立即复投");
                var page_name = "";
        ...
        contract.methods.balanceOf(state.address).call().then(output => {
            ...
            $("#leela_handle").html(math.round(leela_handle, 6));
        ...
        contract.methods.totalSupply().call().then(output => {
            ...
            $("#leela_totalsupply").html(math.round(leela_totalsupply, 6));
...
var state = {
    ...
    game_info: {},
    owner: "",
    tron_domain: "https://bscscan.com",
    wallet_type: "",
    networkType: "injected",
    contract_address: ""
};
...
function waitForGlobal(call_back) {
    ...
    $("#base_header_lian a.lianname").html("币安智能链（BSC）");
    $("#base_header_lian a.lianname").attr("href", "https://bscscan.com/");
    state.tron_domain = "https://bscscan.com";
