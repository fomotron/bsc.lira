        ...
        web3_obj.eth.getAccounts().then(address => {
            if (address.length <= 0) {
                ...
                web3_obj.eth.getBalance(address[0]).then((Balance) => {
                    ...
                    console.log("我的额度:" + Balance_format);
                });
...
function BuyToken() {
    if (state.wallet_type != "web3") {
        ...
        if (!tronWeb.isAddress(state.address)) {
            ...
            var r = confirm("未连接到钱包,请连接钱包后再进行购买!");
            if (r == true) {
                InitMyWallet();
                return;
            }
        ...
        contract.methods.askprice().call().then(output => {
            ...
            WriteLog(output, "当前价格:" + state.price + "BNB");
...
function waitForGlobal(call_back) {
    if (!!window.imToken) {
        ...
        call_back();
        $("#base_header_lian a").html("Tron(波场)");
        ...
        $("#base_header_lian a").html("BSC(安智能链)");
